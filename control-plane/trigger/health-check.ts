  ...
  run: async (payload: { deploymentId: string }) => {
    ...
    try {
      ...
      if (deployment.status === "DEPLOYING") {
        await db.query(`UPDATE deployments SET status='HEALTH_CHECKING',updated_at=now() WHERE id=$1 AND status='DEPLOYING'`,[payload.deploymentId]);
        await db.query(`INSERT INTO deployment_events (deployment_id,from_status,to_status,event_type,message,metadata) VALUES ($1,'DEPLOYING','HEALTH_CHECKING','HEALTH_CHECK_STARTED','Application readiness checks started',$2::jsonb)`,[payload.deploymentId,JSON.stringify({checkUrl,responseBodyStored:false})]);
      }
      ...
      if (attemptNumber > MAX_ATTEMPTS) return { result:"NODE_04_11_ATTEMPTS_EXHAUSTED", deploymentId:payload.deploymentId, attempts:MAX_ATTEMPTS };
      ...
      if(status==="HEALTHY"){
        // HEALTHY is deliberately not LIVE. The verified preview must still be promoted and
        // its unauthenticated production URL verified before the deployment becomes LIVE.
        ...
        await db.query(`INSERT INTO deployment_events (deployment_id,from_status,to_status,event_type,message,metadata) VALUES ($1,'HEALTH_CHECKING','HEALTH_CHECKING','HEALTH_CHECK_PASSED','Application preview is healthy and ready for production promotion',$2::jsonb)`,[payload.deploymentId,JSON.stringify({attemptNumber,httpStatus,latencyMs,checkUrl,responseBodyStored:false})]);
        return {result:"NODE_04_11_HEALTHY",deploymentId:payload.deploymentId,status:"HEALTH_CHECKING",previewUrl:checkUrl,attemptNumber,httpStatus,latencyMs,responseBodyStored:false};
      }

      if(attemptNumber>=MAX_ATTEMPTS){
        await db.query(`UPDATE deployments SET status='FAILED',error_code='HEALTH_CHECK_FAILED',error_message='Application did not become healthy after readiness checks.',finished_at=now(),updated_at=now() WHERE id=$1 AND status='HEALTH_CHECKING'`,[payload.deploymentId]);
      ...
      return {result:"NODE_04_11_RETRY_REQUIRED",deploymentId:payload.deploymentId,status:"HEALTH_CHECKING",attemptNumber,attemptsRemaining:MAX_ATTEMPTS-attemptNumber,httpStatus,latencyMs,errorCode,responseBodyStored:false};
