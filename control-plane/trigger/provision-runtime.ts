...
async function updateRuntimeGitAutoDeployments(projectId:string,body:any){return request(`/v9/projects/${encodeURIComponent(projectId)}${teamQuery()}`,{method:"PATCH",body:JSON.stringify(body)})}
async function enforceRuntimeGitAutoDeployments(project:any){const result=await ensureGitAutoDeploymentsDisabled({project,projectId:project?.id,getProject:getRuntime,updateProject:updateRuntimeGitAutoDeployments});if(!result.ok)throw new Error(`Vercel Git auto-deploy containment failed: ${result.result}`);return result}
async function ensureRuntime({name,repository,rootDirectory,workspaceId,appId,slug}:{name:string;repository:string;rootDirectory:string;workspaceId:string;appId:string;slug:string}){const existing=await getRuntime(name);const action=runtimeRecoveryAction({localRuntime:null,remoteProject:existing});if(action.action==="reconcile-remote-project"){assertRemoteProjectMatchesSscApp(existing,{workspaceId,appId,slug});const gitAutoDeploy=await enforceRuntimeGitAutoDeployments(existing);return{resource:existing,created:false,reconciled:true,gitAutoDeploy};}const normalizedRootDirectory=vercelRootDirectory(rootDirectory);try{const created=await request(`/v11/projects${teamQuery()}`,{method:"POST",body:JSON.stringify({name,framework:"nextjs",...(normalizedRootDirectory?{rootDirectory:normalizedRootDirectory}:{}),gitRepository:{type:"github",repo:repository},...sscManagedProjectGitSettings()})});assertRemoteProjectMatchesSscApp(created,{workspaceId,appId,slug});const gitAutoDeploy=await enforceRuntimeGitAutoDeployments(created);return{resource:created,created:true,reconciled:false,gitAutoDeploy}}catch(error:any){if([400,409].includes(error.status)){const reconciled=await getRuntime(name);if(reconciled){assertRemoteProjectMatchesSscApp(reconciled,{workspaceId,appId,slug});const gitAutoDeploy=await enforceRuntimeGitAutoDeployments(reconciled);return{resource:reconciled,created:false,reconciled:true,gitAutoDeploy}}}throw error}}
