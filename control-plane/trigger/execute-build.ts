...
export const executeBuild=task({id:"ssc-control-plane-execute-build",retry:{maxAttempts:3,minTimeoutInMs:2000,maxTimeoutInMs:10000,factor:2,randomize:false},run:async(payload:{deploymentId:string})=>{
 ...
 try{
  ...
  const body:any={name:deployment.provider_project_name,project:deployment.provider_project_id,target:"production",gitSource:{type:"github",org,repo,ref:deployment.commit_sha},meta:{sscDeploymentId:payload.deploymentId,sscManifestSha256:deployment.manifest_sha256,sscSourceCommitSha:deployment.commit_sha},projectSettings:{framework:deployment.framework||"nextjs",installCommand:deployment.install_command,buildCommand:deployment.build_command}};
  let created:any;try{created=await vercelRequest(`/v13/deployments${teamQuery()}`,{method:"POST",body:JSON.stringify(body)})}catch(error:any){const classified=classifyProviderError(error);if(!classified)throw error;await db.query(`UPDATE deployments SET error_code=$1,error_message=$2,updated_at=now() WHERE id=$3`,[classified.errorCode,classified.userMessage,payload.deploymentId]);await db.query(`INSERT INTO deployment_events (deployment_id,from_status,to_status,event_type,message,metadata) VALUES ($1,'BUILDING','BUILDING','BUILD_PROVIDER_BLOCKED',$2,$3::jsonb)`,[payload.deploymentId,classified.userMessage,JSON.stringify({provider:"vercel",classification:classified.classification,errorCode:classified.errorCode,retryableNow:classified.retryableNow})]);return{result:`NODE_04_10_${classified.classification}`,deploymentId:payload.deploymentId,provider:"vercel",sourceCommitSha:deployment.commit_sha,deploymentStatus:deployment.status,errorCode:classified.errorCode,message:classified.userMessage,retryableNow:classified.retryableNow}}
  const providerDeploymentId=created?.id;if(!providerDeploymentId)throw new Error("Vercel deployment creation returned no deployment id");const providerDeploymentUrl=created?.url?`https://${created.url}`:null;const providerStatus=created?.readyState??created?.status??"QUEUED";
  await db.query("BEGIN");try{await db.query(`INSERT INTO deployment_builds (deployment_id,provider,provider_deployment_id,provider_deployment_url,source_commit_sha,status) VALUES ($1,'vercel',$2,$3,$4,$5)`,[payload.deploymentId,providerDeploymentId,providerDeploymentUrl,deployment.commit_sha,providerStatus]);await db.query(`UPDATE deployments SET provider_deployment_id=$1,error_code=NULL,error_message=NULL,updated_at=now() WHERE id=$2 AND status='BUILDING'`,[providerDeploymentId,payload.deploymentId]);await db.query(`INSERT INTO deployment_events (deployment_id,from_status,to_status,event_type,message,metadata) VALUES ($1,'BUILDING','BUILDING','BUILD_STARTED','Immutable production provider build started',$2::jsonb)`,[payload.deploymentId,JSON.stringify({provider:"vercel",providerDeploymentId,providerDeploymentUrl,sourceCommitSha:deployment.commit_sha,manifestSha256:deployment.manifest_sha256,providerStatus,target:"production"})]);await db.query("COMMIT")}catch(error){await db.query("ROLLBACK");throw error}
  return{result:"NODE_04_10_BUILD_STARTED",deploymentId:payload.deploymentId,provider:"vercel",providerProjectId:deployment.provider_project_id,providerDeploymentId,providerDeploymentUrl,sourceCommitSha:deployment.commit_sha,manifestSha256:deployment.manifest_sha256,buildStatus:providerStatus,target:"production"};
