  ...
  run: async (payload: { deploymentId: string }) => {
    ...
    try {
      ...
      if (observedSha && !sourceIdentityMatches) {
        ...
        await db.query(`UPDATE deployments SET status='FAILED', error_code='BUILD_SOURCE_MISMATCH', error_message='Provider build source did not match the immutable deployment source.', finished_at=now(), updated_at=now() WHERE id=$1`, [payload.deploymentId]);
        await db.query(
      ...
      const terminalSuccess = providerStatus === "READY";
      const terminalFailure = providerStatus === "ERROR" || providerStatus === "CANCELED";
      ...
      if (terminalSuccess) {
        await db.query(`UPDATE deployments SET status='DEPLOYING', error_code=NULL, error_message=NULL, updated_at=now() WHERE id=$1 AND status='BUILDING'`, [payload.deploymentId]);
        await db.query(
          `INSERT INTO deployment_events (deployment_id,from_status,to_status,event_type,message,metadata)
        ...
        await db.query(`UPDATE deployments SET status='FAILED', error_code='BUILD_FAILED', error_message='Application build failed at the deployment provider.', finished_at=now(), updated_at=now() WHERE id=$1 AND status='BUILDING'`, [payload.deploymentId]);
        await db.query(
          `INSERT INTO deployment_events (deployment_id,from_status,to_status,event_type,message,metadata)
