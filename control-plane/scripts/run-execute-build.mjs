...
try {
  ...
  if (deployment.status !== "BUILDING") throw new Error(`Node 04.10 requires BUILDING; current status is ${deployment.status}`);
  const handle = await tasks.trigger("ssc-control-plane-execute-build", { deploymentId: deployment.id });
  console.log(JSON.stringify({
    result: "NODE_04_10_TRIGGERED",
