...
try {
  const result = await db.query(
    `SELECT d.id, d.deployment_key, d.status
       FROM deployments d
  ...
  const handle = await tasks.trigger("ssc-control-plane-enforce-resource-policy", { deploymentId: deployment.id });
  console.log(JSON.stringify({
    result: "NODE_04_14_TRIGGERED",
    appSlug,
