...
try {
  ...
  if (deployment.status !== "ANALYZING") throw new Error(`Node 04.17 verification requires ANALYZING; current status is ${deployment.status}`);

  const handle = await tasks.trigger("ssc-control-plane-verify-env-requirements", {
    deploymentId: deployment.id,
  });

  console.log(JSON.stringify({
    result: "NODE_04_17_ENV_VERIFICATION_TRIGGERED",
    appSlug,
    deploymentId: deployment.id,
