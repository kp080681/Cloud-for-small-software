...
try {
  ...
  if (deployment.status !== "BUILDING") throw new Error(`Node 04.9 requires BUILDING; current status is ${deployment.status}`);
  const handle = await tasks.trigger("ssc-control-plane-apply-runtime-env", { deploymentId: deployment.id });
  console.log(JSON.stringify({
    result: "NODE_04_9_TRIGGERED",
    appSlug,
